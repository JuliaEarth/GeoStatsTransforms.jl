@testset "InterpolateLocal" begin
  @test !isrevertible(InterpolateLocal(CartesianGrid(2, 2)))

  pset = PointSet(rand(Point2, 3))
  gtb = georef((a=[1, 2, 3], b=[4, 5, 6]), pset)
  ngtb = gtb |> InterpolateLocal(pset, maxneighbors=3)
  @test ngtb.a == gtb.a
  @test ngtb.b == gtb.b
  ...
  Random.seed!(2021)
  ngtb = gtb |> InterpolateLocal(grid, :z => Kriging(variogram), maxneighbors=3)
  ...
  Random.seed!(2021)
  ngtb = gtb |> InterpolateLocal(grid, :z => Kriging(variogram), maxneighbors=3, neighborhood=MetricBall(100.0))
  @test isapprox(ngtb.z[linds[25, 25]], 1.0, atol=1e-3)
  @test isapprox(ngtb.z[linds[50, 75]], 0.0, atol=1e-3)
  @test isapprox(ngtb.z[linds[75, 50]], 1.0, atol=1e-3)
end
